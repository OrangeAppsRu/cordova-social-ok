...
- (id) initWithErrorBlock: (OKErrorBlock) errorBlock {
    if(self = [super init]) {
...
-(id)initWithAppIdAndAppKey:(NSNumber *)appId appKey:(NSString *)appKey {
    if(self = [super init]) {
        self.queue = [[NSOperationQueue alloc] init];
        self.queue.name = @"OK-API-Requests";
        self.queue.maxConcurrentOperationCount = OK_MAX_CONCURRENT_REQUESTS;
        self.appId = [appId stringValue];
        self.oauthRedirectUri = [NSString stringWithFormat:@"ok%@://authorize", self.appId];
        self.appKey = appKey;
        self.completitionHandlers = [NSMutableDictionary new];
        NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
        self.accessToken = [userDefaults objectForKey:OK_USER_DEFS_ACCESS_TOKEN];
        self.accessTokenSecretKey = [userDefaults objectForKey:OK_USER_DEFS_SECRET_KEY];
    }
    return self;
}
...
-(void)invokeMethod:(NSString *)method arguments:(NSDictionary *)methodParams session:(bool)sessionMethod signed:(bool)signedMethod success:(OKResultBlock)successBlock error:(OKErrorBlock) errorBlock {
    ...
    [arguments setValuesForKeysWithDictionary:@{@"application_key":self.appKey, @"method":method, @"format":@"json"}];
...
-(void)showWidget:(NSString *)command arguments:(NSDictionary *) arguments options:(NSDictionary *)options success:(OKResultBlock)successBlock error:(OKErrorBlock) errorBlock {
    NSString *returnUri = [NSString stringWithFormat:@"ok%@://%@",self.appId, command];
    NSString *widgetUrl = [NSString stringWithFormat:@"%@%@&%@%@",OK_WIDGET_URL,[command ok_encode],[[arguments ok_union: @{@"st.redirect_uri":returnUri}]ok_queryStringWithSignature:self.accessTokenSecretKey sigName:@"st.signature"],[[options ok_union: @{@"st.access_token":self.accessToken,@"st.app":self.appId,@"st.nocancel":@"on"}] ok_queryString]];
...
+(void)initWithAppIdAndAppKey:(NSNumber *)appId appKey:(NSString *) appKey{
    connection = [[OKConnection alloc] initWithAppIdAndAppKey:appId appKey:appKey];
}
